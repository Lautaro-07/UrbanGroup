...
switch ($action) {
    case 'add':
        if ($propertyId > 0) {
            $result = $favoriteModel->addFavorite($clientId, $propertyId);
            if ($result) {
    ...
    case 'toggle':
        if ($propertyId > 0) {
            ...
            if ($isFavorite) {
                $favoriteModel->removeFavorite($clientId, $propertyId);
                echo json_encode(['success' => true, 'is_favorite' => false, 'message' => 'Eliminado de favoritos']);
            } else {
                $favoriteModel->addFavorite($clientId, $propertyId);
                echo json_encode(['success' => true, 'is_favorite' => true, 'message' => 'Agregado a favoritos']);
            }
        } else {
            echo json_encode(['success' => false, 'message' => 'ID de propiedad inválido']);
        }
