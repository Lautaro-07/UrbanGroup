...
class PhotoModel {
    ...
    public function getByPropertyId($propertyId) {
        $stmt = $this->db->prepare("SELECT id, property_id, url as photo_url, is_main, sort_order as display_order, created_at FROM property_photos WHERE property_id = ? ORDER BY sort_order ASC");
