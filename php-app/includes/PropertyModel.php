...
class PropertyModel {
    ...
    public function getAll($filters = [], $limit = null, $offset = 0) {
        $sql = "SELECT p.*, c.name as comuna_name, r.name as region_name 
                FROM properties p 
    ...
    public function getById($id) {
        $sql = "SELECT p.*, c.name as comuna_name, r.name as region_name, u.name as partner_name, u.company_name, u.phone as partner_phone, u.email as partner_email
                FROM properties p 
                LEFT JOIN comunas c ON p.comuna_id = c.id 
                LEFT JOIN regions r ON p.region_id = r.id 
    ...
    public function create($data) {
        $sql = "INSERT INTO properties (title, description, property_type, operation_type, price, currency, bedrooms, bathrooms, built_area, total_area, parking_spots, address, comuna_id, region_id, images, features, is_featured, is_active, partner_id) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([
            $data['title'],
    ...
    public function update($id, $data) {
        ...
        $allowedFields = ['title', 'description', 'property_type', 'operation_type', 'price', 'currency', 'bedrooms', 'bathrooms', 'built_area', 'total_area', 'parking_spots', 'address', 'comuna_id', 'region_id', 'images', 'features', 'is_featured', 'is_active'];
